...
namespace optimize {
  ...
  public:
    ...
    {
      /* return (1.0 - pow(1.0 - static_cast<double>(x) / static_cast<double>(k),
     * h)); */
    ...
    double mpd(const double d, const double rho)
    {
      double npm = 0;
      double dpm = 0;
#pragma omp critical
      {
        for (uint32_t x = 0; x <= k; ++x) {
          ...
          double xa = static_cast<double>(x > hdist_th) +
                      static_cast<double>(x <= hdist_th) * (1.0 - static_cast<double>(binom_coef_k[x] - binom_coef_hnk[x]) /
                                                                    static_cast<double>(binom_coef_k[x]));
    ...
    double mpdd(const double d, const double rho)
    {
      double gd = 0;
      double ffd = 0;
      double fpd = 0;
...
#pragma omp critical
      {
        for (uint32_t x = 0; x <= k; ++x) {
          ...
          double v2 = (xx * xx + (k - 1) * k * d * d - x * (1 + (k - 1) * 2 * d)) / ((d * (1 - d)) * (d * (1 - d)));

          gd += xa * binom_coef_k[x] * p2 * v;
          gpd += xa * binom_coef_k[x] * p2 * v2;
          ffd += (p2 * binom_coef_k[x] * xa);
          fpd += (p2 * binom_coef_k[x] * v * xa);
        }
      }
      gd = gd * rho;
      fpd = fpd * rho;
      gpd = gpd * rho;
      double fd = (1.0 - rho + rho * ffd);
      return (((fd * gpd) - (gd * fpd)) / (fd * fd));
    }

    double smd(double dist_th, uint32_t hdist)
    {
      return (static_cast<double>(hdist) - k * dist_th) / (dist_th * (1 - dist_th));
    }

    double smdd(double dist_th, uint32_t hdist)
    {
      return (static_cast<double>(hdist) * (2 * dist_th - 1) - k * dist_th * dist_th) /
             (dist_th * dist_th * (1 - dist_th) * (1 - dist_th));
    }
